package com.trablone.csscreated.widgets;
...
public class ShaderEditor extends EditText
{
	...
	private static final int COLOR_METOT = 0x0E126AD6;

	private static final Pattern line = Pattern.compile(
		".*\\n" );
	private static final Pattern numbers = Pattern.compile(
		"\\b(\\d*[.]?\\d+)\\b" );
	private static final Pattern css_atribute = Pattern.compile(
		"\\b(-|text|top|transform|align|clear|color|max|margin|position|padding|float|family|font|shadow|style|space|size|word|webkit|white|width|weight|wrap|"+
		"background|box|border|bottom|radius|decoration|display|left|list|line|height|vertical|z|index|)\\b");
	...
	public void setTextHighlighted(final CharSequence text )
	{
		cancelUpdate();
		...
		h.post(new Runnable(){
				...
				public void run()
				{
					setText(text);//highlight( new SpannableStringBuilder( text ) ) );
	...
	private Editable highlight( Editable e )
	{
		try
		{
			...
			for( Matcher m = css_atribute.matcher( e );
				m.find(); )
				e.setSpan(
